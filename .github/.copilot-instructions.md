...
- When testing changes, restart server in background: `python api_logic_server_run.py &`
...
..Item[None] {Formula unit_price} unit_price: 105.0
....SysSupplierReq[None] {Insert - Supplier AI Request} product_id: 6, chosen_unit_price: None
....SysSupplierReq[None] {Event - calling AI} chosen_unit_price: [None-->] 105.0
..Item[None] {Formula amount} amount: 1050.0
..Item[None] {adjust parent Order.amount_total}
....Order[1] {Update - Adjusting order: amount_total} amount_total: [300.0-->] 1350.0
```

**Key formatting rules:**
- `..` prefix = nesting level (2 dots = Item, 4 dots = child/nested object)
- Each line shows: `Class[id] {action/reason} attribute changes`
- Value changes shown as: `[old_value-->] new_value`
- Hierarchical indentation shows call depth and parent-child relationships
- Event handlers and nested inserts shown at deeper indent levels
...
- When AI logic generation creates new models (e.g., audit tables for probabilistic rules), these models MUST be added to database/models.py
- Alembic migrations must be run to create the tables in the database
- Stop the server before running migrations to avoid database locking issues
- After migrations complete, restart the server to load the new models
